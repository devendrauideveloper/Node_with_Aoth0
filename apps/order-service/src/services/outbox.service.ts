...
import { getPendingOutboxBatch, markOutboxPublished } from "../repositories/order.repository.js";
...
export function startOutboxPublisher() {
  setInterval(() => {
    void publishOutboxBatch().catch((error) => logger.error({ err: error }, "order outbox publish failed"));
  }, 1500);
}
