...
import { getPendingOutboxBatch, markOutboxPublished } from "../repositories/inventory.repository.js";
...
export function startOutboxPublisher() {
  setInterval(() => {
    void publishOutboxBatch().catch((error) => logger.error({ err: error }, "inventory outbox publish failed"));
  }, 1500);
}
